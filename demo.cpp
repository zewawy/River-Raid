...
int main(){
	window w(1200, 700, 5, 5);
	w.ChangeTitle("plane body");
	w.SetPen(LIGHTGRAY,5);
	w.DrawLine(960, 370, 1040, 370);
	w.SetPen(BLACK);
	w.DrawRectangle(980, 400, 1021, 451); 
	w.DrawArc(980, 430, 1020, 370, 0, 180,FILLED); 
	w.DrawTriangle(980, 450, 1020, 450, 1000, 490);  
	w.DrawTriangle(993, 475, 993, 500, 950, 500); 
	w.DrawTriangle(1007, 475, 1007, 500, 1050, 500);  
	w.DrawRectangle(993, 475, 1008, 501);

	int ipX[] = { 980, 980, 900, 900 };  
	int ipY[] = { 405, 445, 430, 420 };  
	w.DrawPolygon(ipX, ipY, 4, FILLED); 
	
	int ipm[] = { 1020, 1020, 1100, 1100 }; 
	int ipn[] = { 405, 445, 430, 420 }; 
	w.DrawPolygon(ipm, ipn, 4, FILLED);   
	



    int hullX[] = { 500, 800, 750, 650, 550 };
    int hullY[] = { 400, 400, 450, 480, 450 };
    w.SetPen(BLACK, 5);
    w.DrawPolygon(hullX, hullY, 5, FILLED);
    w.SetPen(GRAY, 5);
    w.DrawRectangle(550, 370, 800, 400);

    w.SetPen(BLACK, 5);
    w.DrawLine(650, 370, 650, 250);

    w.SetPen(BLACK, 5);
    w.DrawTriangle(650, 250, 650, 370, 750, 370, FILLED);

	w.SetPen(RED, 5);
    w.DrawTriangle(650, 250, 640, 260, 650, 260, FILLED);



    w.SetPen(GRAY, 5);
    w.DrawRectangle(50, 400, 450, 500);

    w.SetPen(BLACK, 5);
    w.DrawRectangle(150, 350, 300, 500);

    w.SetPen(BLACK, 8);
    w.DrawLine(300, 425, 500, 425);

    w.SetPen(BLACK, 3);
    int wheelRadius = 30;
    int wheelCentersX[] = { 100, 200, 300, 400 };
    int wheelCenterY = 520;
    for (int i = 0; i < 4; i++) {
        int cx = wheelCentersX[i];
        int cy = wheelCenterY;
        w.DrawArc(cx - wheelRadius, cy - wheelRadius, cx + wheelRadius, cy + wheelRadius, 0, 360, FILLED);
    }

    w.SetPen(LIGHTGRAY, 5);
    w.DrawArc(200, 360, 260, 420, 0, 360, FILLED);

    w.SetPen(BLACK, 3);
    w.DrawRectangle(40, 450, 60, 465);
